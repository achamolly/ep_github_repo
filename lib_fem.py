...
class ActiveStress1D:
    def __init__(self, margin, t, N, mu, d, ten_profile, baseline_tension, thinterp, l, cumt, t_dep, **kwargs):
        self.margin = margin
        self.t = t
        self.N = N
        self.mu = mu
        self.d = d
        self.ten_profile = ten_profile
        self.baseline_tension = baseline_tension
    ...
    def __call__(self, x):
        gdim = 1
        values = np.zeros((1, x.shape[1]), dtype=PETSc.ScalarType)  # (4,n) matrix with the rows sxx sxy syx syy
        s = tisstens(x, self.margin, self.t, self.N, self.mu, self.d, self.ten_profile, self.baseline_tension,
                     self.thinterp, self.l, self.cumt, self.t_dep)
        values = s[0] + s[3]  # lazy
        return values
...
def accum_ingression(t, mesh):
    """Integral of coef5"""
    # t += 5.1/2
    acc_ing = Constant(mesh, PETSc.ScalarType(-1.72 + 0.390 * t - 0.195 * np.log(1 - np.tanh(5.1 - t) ** 2)))
    return acc_ing


def calc_div(mesh, U, R, Dv=None):
    """Calculates UFL object for accumulated divergence (area changes)"""
    # dv = ufl.div(U)
    dv = Constant(mesh, PETSc.ScalarType(1)) / ufl.det(
        ufl.Identity(2) - ufl.grad(U))  # using the defintion of the deformation gradient tensor
    dv_expr = Expression(dv, R.element.interpolation_points())
    if Dv is None:
        Dv = Function(R)
...
def create_function_spaces(mesh, THorder=1):
    V_el = ufl.VectorElement("Lagrange", mesh.ufl_cell(), THorder + 1)  # velocity, quadratic P2
    R_el = ufl.FiniteElement("Lagrange", mesh.ufl_cell(), THorder)  # density, quadratic P2
    V = FunctionSpace(mesh, V_el)
    R = FunctionSpace(mesh, R_el)
    return V, R
...
def define_initial_conditions(V, R, initdens="uniform"):
    U = Function(V)
    U.x.array[:] = 0  # displacements, initially zero
    v_sol = Function(V)  # velocity solution, initially zero
    Rho = Function(R)
    Rho.x.array[:] = 1  # intial densities are uniform
    return U, Rho, v_sol


def density_remesh(newmesh, oldmesh, Rho, THorder):
    ...
    newR = FunctionSpace(newmesh, ufl.FiniteElement("Lagrange", newmesh.ufl_cell(), THorder))
    newRho = Function(newR)
    Rho_expr = EpibolyInterpolator(Rho, oldmesh)
    newRho.interpolate(Rho_expr)  # interpolate onto new mesh
    return newRho
...
def det_newradius_retract(mesh, rtol=1e-2, trim=1e-3):
    """Trim currentradius to make sure new mesh lies entirely within the old one"""
    meshnorms = np.linalg.norm(mesh.geometry.x, axis=1)  # L2 norm of mesh vertex positions
    newcurrentradius = np.min(meshnorms[np.isclose(meshnorms, np.max(meshnorms), rtol)]) * (
            1 - trim)  # new radius is just within old mesh
    return newcurrentradius


def displacement_remesh(newmesh, oldmesh, U, THorder):
    """interpolate area changes from old mesh onto new mesh"""
    # Include custom EpibolyInterpolator class for missing dolfinx feature of direct interpolation on different meshes.
    newV = FunctionSpace(newmesh, ufl.VectorElement("Lagrange", newmesh.ufl_cell(), THorder + 1))
    newU = Function(newV)
    U_expr = EpibolyInterpolator(U, oldmesh)
    newU.interpolate(U_expr)  # interpolate onto new mesh
    return newU


def edge_vel(t):
    """Edge velocity (from Arthur's data)"""
    un = 0.115 * (1 - np.exp(-t / 2.0))
    return un
...
def evolve_densities_and_displacement(mesh, U, Rho, R, v_sol, dt):
    # calculate density change
    rhod = -Rho * ufl.div(v_sol)
    rhod_expr = Expression(rhod, R.element.interpolation_points())
    Rhod = Function(R)
    Rhod.interpolate(rhod_expr)

    # update densities and displacement
    Rho.x.array[:] += Rhod.x.array[:] * dt
    U.x.array[:] += v_sol.x.array[:] * dt
    return Rho, U
...
def get_stressobj(margin, t, N, mu, d, ten_profile, baseline_tension, interpolators, l, cumt, t_dep):
    """Create stressobj to be interpolated onto appropriate FE space"""
    cinterp, Tinterp, thinterp = interpolators
    stressobj = ActiveStress(margin, t, N, mu, d, ten_profile, baseline_tension, thinterp, l, cumt, t_dep)
    return stressobj


def get_stressobj1D(margin, t, N, mu, d, ten_profile, baseline_tension, interpolators, l, cumt, t_dep):
    """Create stressobj1D to be interpolated onto appropriate FE space"""
    cinterp, Tinterp, thinterp = interpolators
    stressobj1D = ActiveStress1D(margin, t, N, mu, d, ten_profile, baseline_tension, thinterp, l, cumt, t_dep)
    return stressobj1D
...
def init_writers(prefix, U, Rho, v_sol, Dv, mesh):
    """For VE only atm"""
    if os.path.exists(prefix):
        shutil.rmtree(prefix)
    os.mkdir(prefix)

    vtx_u = VTXWriter(mesh.comm, prefix + "/U.bp", [U])  # displacements
    vtx_rho = VTXWriter(mesh.comm, prefix + "/Rho.bp", [Rho])  # Density
    vtx_v = VTXWriter(mesh.comm, prefix + "/V.bp", [v_sol])  # Velocities
    vtx_div = VTXWriter(mesh.comm, prefix + "/Div.bp", [Dv])  # instantaneous divergence
    vtxs = [vtx_u, vtx_rho, vtx_v, vtx_div]
    return vtxs
...
def regulate_margin(N, ps, u_values, gd, c, lt, T, Ms, coefs, mesh, regulation_parameters, regulation_settings, dt,
    ...
    if Ms is not None:
        ...
        extension_rate_correction = np.zeros(N)
    ...
    if regulate:
        for it in range(subd):
            for i in range(N):
                ...
                dT[i] = c0 * Ts / (lam * emax * ve_time) * (gd[i] - extension_rate_correction[i] - emax * np.tanh(lam * (T[i] / c[i] / Ts - 1)))
            dc[N] = dc[0]
            dT[N] = dT[0]
            c = c + dt / subd * dc
            T = T + dt / subd * dT
...
def tisstens(x, margin, t, N, mu, d, ten_profile, baseline_tension, thinterp, l, cumt, t_dep='const'):
    """Calculate tension at point x according to Gaussian profile with interpolation. Return active stress tensor"""
    """Implementation for static tension profile"""
    s = np.zeros((4, x[0].size))
    c = geom.Polygon(margin).centroid  # margin center of mass
    c_coords = np.array([c.x, c.y])

    points = np.column_stack((x[0], x[1]))  # Combine x and y coordinates

    dist = np.zeros_like(x[0])  # Initialize arrays for distance and almarg
    almarg = np.zeros_like(x[0])

    for i, (x, y) in enumerate(points):
        xp = geom.Point(x, y)  # Create a shapely Point for each coordinate pair
        dist[i] = margin.distance(xp)  # distance from margin
        almarg[i] = margin.project(xp)  # distance along margin (absolute)

    the = thinterp((almarg - l[0] / 2) % cumt[N])

    tax = np.array([np.cos(the), np.sin(the)])

    maxTension = 2.7 * mu * (1 + np.tanh(2 * (t - 0.95) / 2.9)) / 2 * np.exp(-t / 8.3)  # peak tension, time in hours

    if ten_profile == "syn":
        # Gaussian tension profile
        a = np.arctan2(points[:, 0] - c_coords[0], c_coords[1] - points[:, 1])
        T = np.exp(-np.square(a / 0.698) / 2)
        T = T * maxTension
    elif ten_profile == "unif":
        # uniform tension, only baseline
        T = maxTension * 0

    # add baseline tension
    assert t_dep in ['const', 'plat', 'lin']
    match t_dep:
        case 'const':
            T = T + baseline_tension
        case 'plat':
            t_reg = 0.5
            T = T + baseline_tension * (1 - np.exp(-t / t_reg))
        case 'lin':
            t_reg = 0.5
            T = T + baseline_tension * (t - t_reg * (1 - np.exp(-t / t_reg)))

    # Add lateral extent
    ten = 1.0 / np.sqrt(2.0 * np.pi * pow(d, 2)) * np.exp(-pow(dist, 2) / (2.0 * d * d)) * T

    # Define stress tensor
    s[0] = tax[0] * tax[0] * ten
    s[1] = tax[0] * tax[1] * ten  # off diagonal
    s[2] = tax[0] * tax[1] * ten  # off diagonal
    s[3] = tax[1] * tax[1] * ten
...
def VE_femsolve(t, U, V, N, mesh, markers, Ms, coefs, stressobj, stressobj1D, acc_ing, tissue_parameters, fem_settings,
...
#
# def zip_FEM(dataDir):
#     folder_path = dataDir + '/FEMData'
#     zip_file_path = dataDir + '/FEMdata.zip'
#     try:
#         # Create a ZipFile object in write mode
#         with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
#             # Walk through the folder and add all files and directories to the zip file
...
#
#         # Once the zip file is created, delete the original folder
#         shutil.rmtree(folder_path)
#
#         print(f"Folder '{folder_path}' has been zipped to '{zip_file_path}' and deleted.")
#
